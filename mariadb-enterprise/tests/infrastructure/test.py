...
def testStatefulSet():
    ...
    stream(v1.connect_get_namespaced_pod_exec, umPod.metadata.name, umPod.metadata.namespace, command=exec_command, container=container, stderr=True, stdin=False, stdout=True, tty=False)
    ...
    resp = stream(v1.connect_get_namespaced_pod_exec, umPod.metadata.name, umPod.metadata.namespace, command=exec_command, container=container, stderr=True, stdin=False, stdout=True, tty=False)
